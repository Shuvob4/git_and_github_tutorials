...
Once we are in the desired directory, we will initialize our project
...
`git merge` command will merge the changes from one branch into the
current branch.
...
We can combine these commands together as well, i.e. `git clean -dn` to
